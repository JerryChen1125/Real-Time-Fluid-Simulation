...
namespace FluidSimulation
{
	...
	void InspectorView::display()
	{
		...
		bool isLagrangian2d = Manager::getInstance().getMethod() != NULL &&
			Manager::getInstance().getMethod()->id == 0;
		if (isLagrangian2d)
		{
			bool disabledSpring = simulating;
			if (disabledSpring)
			{
				ImGui::BeginDisabled();
			}
			...
			{
				glfwMakeContextCurrent(window);
				Lagrangian2dPara::enableFountain2d = !Lagrangian2dPara::enableFountain2d;
				Manager::getInstance().getMethod()->init();
				Manager::getInstance().getSceneView()->texture = -1;
				simulating = false;
				if (Lagrangian2dPara::enableFountain2d)
				{
					Glb::Logger::getInstance().addLog("Lagrangian 2d spring ON.");
				}
				else
				{
					Glb::Logger::getInstance().addLog("Lagrangian 2d spring OFF.");
				}
			}
